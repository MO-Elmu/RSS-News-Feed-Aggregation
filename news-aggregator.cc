...
#include "semaphore.h"
using namespace std;
static unsigned short numFeedThreads = 5;
static unsigned short numArticleThreads = 8;
static unsigned short numTotalThreads = 18;
...
void NewsAggregator::buildIndex() {
  ...
  xmlCatalogCleanup();
  xmlCleanupParser();
  //for(auto& p : serverLinksMap)  p.second.reset();
}
...
void NewsAggregator::getRssFeeds(string url, semaphore& s){
	RSSFeed feed(url);
	try{feed.parse();}
	catch(const RSSFeedException& e){s.signal(on_thread_exit);} 
	...
	semaphore totalAllowed(numTotalThreads);
	for(auto const& article : articleSet){
                SetLock.lock();
                if(urlTracker.find(article) != urlTracker.end()) {SetLock.unlock(); continue;}
                SetLock.unlock();
		totalAllowed.wait();
		articleThreads.push_back(thread([this](Article artcl, semaphore& limit){
			this->getFeedArticles(artcl, limit);
		}, article, ref(totalAllowed)));
	}
        SetLock.lock();
...
void NewsAggregator::getFeedArticles(Article artcl, semaphore& limit){
	MapLock.lock();
	unique_ptr<semaphore>& up = serverLinksMap[getURLServer(artcl.url)]; //get the semaphore for this server
	if(up == nullptr) up.reset(new semaphore(numArticleThreads));
	MapLock.unlock();
	up->wait();
	HTMLDocument htmlDoc(artcl.url);
	try{htmlDoc.parse();}
	catch(const HTMLDocumentException& e){up->signal(on_thread_exit); limit.signal(on_thread_exit);}
	const vector<string>& words = htmlDoc.getTokens();
	IndexLock.lock();
	index.add(artcl, words);
	IndexLock.unlock();
	up->signal(on_thread_exit);
	limit.signal(on_thread_exit);
}
...
void NewsAggregator::processAllFeeds() {
	...
	try{
 	...
 	const auto& feeds = rssFeedList.getFeeds();
        vector<thread> feedListReaders;
        semaphore numAllowed(numFeedThreads);
 	for (auto const& feed_info : feeds){
                numAllowed.wait();
                feedListReaders.push_back(thread([this](string url, semaphore& s){
                        this->getRssFeeds(url,s);
                }, feed_info.first, ref(numAllowed)));
        }
        for (thread& t: feedListReaders) t.join();
        index.updateIndex();
}
